...
public interface IEngage_interviewDao {
	...
	@Insert("insert into Engage_interview value(null,#{human_name},"
			+ "1,#{human_major_kind_id},#{human_major_kind_name},#{human_major_id},"
			+ "#{human_major_name},#{image_degree},#{native_language_degree},"
			+ "#{foreign_language_degree},#{response_speed_degree},#{eQ_degree},"
			+ "#{iQ_degree},#{multi_quality_degree},#{register},#{checker},#{registe_time},"
			+ "#{check_time},#{resume_id},#{result},#{interview_comment},#{check_comment},2,0)")
	...
	@Update("update Engage_interview set checker=#{checker},check_time=#{check_time},"
			+ "check_status=#{check_status} where ein_id=#{ein_id}")
	...
	@Update("update Engage_resume set pass_checkComment=#{pass_checkComment}"
			+ " where res_id=#{res_id}")
